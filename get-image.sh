...
sudo cp -r ~/new_image.jpg /var/www/html/image.jpg

# copy again with timestamp to archive
now=$(date "+%F_%H-%M-%S")
# create folder if not exist: /var/www/html/images/
sudo mkdir -p /var/www/html/images
sudo cp -r ~/new_image.jpg /var/www/html/image_$now.jpg

# get last 100 log lines
tail -n 100 ~/get-image.log >~/imagelog.txt

# copy to /var/www/html/imagelog.txt
sudo cp -r ~/imagelog.txt /var/www/html/imagelog.txt
